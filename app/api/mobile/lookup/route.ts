 ...
 */
// Аломатҳои канорӣ — нохунак, қавс, нуқта, фосила.
//
// 🔴 БОГИ ЧЕНАКШУДА (2026-09-09): намунаи пештара
//
//     /^[^0-9A-Za-zЀ-ӿ'-]+|[^0-9A-Za-zЀ-ӿ'-]+$/g
//
// «ҳар чизе, ки рақам, лотинӣ ё кириллӣ нест»-ро мебурид. Дар калимаи
// кореягӣ, арабӣ, чинӣ, японӣ, ибрӣ ё тайландӣ ҳеҷ яке аз онҳо НЕСТ — пас
// ТАМОМИ калима «партов» ҳисоб мешуд ва сатри холӣ мемонд:
//
//     «가세요» → ""  →  {"error":"word is required"}
//
// Хонанда китоби кореягиро мекушод, ба калима мезад, варақа мебаромад — ва
// холӣ мемонд. Ҳеҷ хато, ҳеҷ огоҳӣ.
//
// ⚠️ Акнун танҳо АЛОМАТ бурида мешавад (`P` китобат, `S` рамз, `Z` фосила,
// `C` идоракунӣ), на «ҳама чизи ношинос». Ҳарф, рақам ва аломатҳои
// ҲАМРОҲ (`M`) дар ҳама хат нигоҳ дошта мешаванд — вагарна тайии тайландӣ
// ва ҳиндӣ бурида мешуд: «สวัสดี» → «สวัสด».
// ⚠️ Тавассути `new RegExp` сохта мешавад, на литерал: `tsconfig` ба ES5
// нишон мегирад ва флаги `u`-и литералӣ он ҷо хатои компилятсия медиҳад
// (TS1501). Дар иҷро Node онро дастгирӣ мекунад.
const EDGE_JUNK = new RegExp(
  '^[\\p{P}\\p{S}\\p{Z}\\p{C}]+|[\\p{P}\\p{S}\\p{Z}\\p{C}]+$',
  'gu',
);
