...
const cards = JSON.parse(readFileSync('cards_v4.json', 'utf8'));
...
const html = `<section class="wrap">
  <header class="head">
    <p class="eyebrow">RAMZ Academy — Flat Vector, версияи қатъӣ</p>
    <h1>Бе чеҳра · Бе градиент · Замина якхела</h1>
    <p class="sub">Ҳамон 8 калима, бо қоидаҳои нав: танҳо flat vector, ягон эмотсия/чеҳра ба ашё ё ҳайвон, бетарафии фарҳангӣ, заминаи якхелаи бетараф, палитраи ягонаи pastel.</p>
  </header>
  ...
  <footer class="foot">
    <p>Ҳар расм 512×512, ~6–13KB — хеле сабук барои офлайн/prefetch.</p>
  </footer>
</section>
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    :root {
      ...
      --accent: #34e0c6; --accent-soft: rgba(52, 224, 198, 0.14);
  ...
  :root[data-theme="light"] {
    ...
    --accent: #0e9c8c; --accent-soft: rgba(14, 156, 140, 0.10);
  ...
  * { box-sizing: border-box; }
  ...
  .word { font-size: 18px; font-weight: 800; letter-spacing: -0.01em; }
  .tj { font-size: 14px; color: var(--sub); font-weight: 500; }
...
writeFileSync('gallery4.html', html);
console.log('gallery4.html тайёр, ' + (Buffer.byteLength(html) / 1024).toFixed(0) + 'KB');
