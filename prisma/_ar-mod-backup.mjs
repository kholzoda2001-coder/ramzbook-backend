// Нусхаи эҳтиётии ҳамаи сатрҳои як модул (умумӣ).
//
//   node prisma/_ar-mod-backup.mjs 1
...
const [m] = await sql.query(`SELECT id, "canDoStatement" FROM "Module" WHERE "courseId"=$1 AND "order"=$2`, [c.id, Number(process.argv[2] ?? 0)]);
...
writeFileSync(`tmp/ar-m${Number(process.argv[2] ?? 0)}-backup.json`, JSON.stringify(out, null, 1));
