...
const REPO = `${process.env.TEMP}/ramz-audio-img4`.split('\\').join('/');
...
const FILES = [
  ['الاثنين.png', 'Душанбе — ШАҲРИ Душанбе нишон дода мешавад, вале корт РҮЗи ҳафта аст'],
  ['بعد_الظهر.png', 'Баъд аз зӯҳр — матни бемаънии сохта дар поён'],
  ['المساء.png', 'Шом — матни бемаънии сохта дар поён'],
];
...
if (!git(['status', '--porcelain']).trim()) console.log('\nтағйирот нест');
  ...
  git(['-c', 'user.name=RAMZ Content', '-c', 'user.email=help@ramz.tj', 'commit', '-m',
    BACK ? 'Arabic images: restore Module 4 pictures'
         : 'Arabic Module 4: park 3 wrong/garbled pictures so the app falls back to emoji']);
