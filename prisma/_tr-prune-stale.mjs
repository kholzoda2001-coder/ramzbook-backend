...
//
// `_tr-module-build.mjs` танҳо илова ва кӯчонида метавонад — вай ҳеҷ гоҳ
...
//
//   node prisma/_tr-prune-stale.mjs [--apply]
...
// Дарси луғате, ки файли мазмун ДИГАР намешиносад (мас. баъди иваз кардани
// номи дарс): калимаҳояш кӯҳнаанд ва худи дарс баъд холӣ мемонад.
const knownTitles = new Set([...want.keys()]);
for (const r of rows) {
  if (r.st !== 'vocab') continue;
  if (!knownTitles.has(`M${r.mo}|${r.title}`) && !stale.includes(r)) stale.push(r);
}
