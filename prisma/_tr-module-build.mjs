...
// IPA ва хониши тоҷикӣ аз ХУДИ ИМЛОИ ТУРКӢ ҳисоб мешаванд — туркӣ фонетикӣ аст.
// Пештар ин ҷо `ipaToTajik = (ipa) => ipa` буд, яъне сутуни «хониши тоҷикӣ»
// худи IPA-ро нигоҳ медошт ва хонанда «/byjyc/» мебинад, на «бюйю́к».
...
// Файлҳои мазмун `emoji` доранд, на `icon` — бе ин сатр эмоҷии бахш 📚 мешуд.
...
const WORK = `tmp/tr-m${MODULE.order + 1}-audio`;
...
// Як матни калима метавонад ЧАНД сатр дошта бошад: дарси навиштан қасдан
// нусха мегирад. Агар харита танҳо ЯК сатрро нигоҳ дорад, билд метавонад маҳз
// нусхаи дарси навиштанро ба дарси луғат кӯчонад ва такрор ба вуҷуд оварад.
...
for (const item of created.filter(c => c.vocab)) {
  ...
  for (const w of item.vocab.words) {
    if (w.existing) {
      ...
      // Майдони `ipa`-и файли мазмун сарфи назар мешавад: он дастӣ навишта шуда
      // буд ва хато дошт (ğ → «.»). Манбаи ягона — имлои калима.
      ...
      if (!DRY) {
        const { word } = await api('words', 'POST', {
          ...
          ipa, ipaTajik: tajik, example: w.example, exampleTrans: w.exampleTrans, order: i,
        });
        audioJobs.push({ id: word.id, text: w.word, kind: 'word' });
        madeWords++;
      } else {
        console.log(`    нав: ${w.word.padEnd(18)} ${ipa.padEnd(22)} → ${tajik}`);
      }
    }
    i++;
  }
...
if (!DRY && audioJobs.length) {
  mkdirSync(WORK, { recursive: true });
    ...
    // TTS va Upload muwaqqatan bozdashta shudaast
    // python prisma/_tr-tts.py ...
    console.log(`  Audio TTS items saved to ${WORK}/items.json`);
    console.log(`  Skipping audio upload because Vercel Blob is suspended.`);

    // Sync writing lesson audio URLs
    ...
    if (synced) console.log(`  Аудиои дарси навиштан нусха шуд: ${synced}`);
  ...
  } else console.log(`  ${audioJobs.length} сатр омода`);
  
  if (!DRY) {
    console.log('\n== Ҳиссаи нутқ ==');
    // console.log('  ' + execFileSync('node', ['prisma/_tr-pos.mjs'], { encoding: 'utf8', maxBuffer: 1 << 24 }).trim().split('\n').slice(-1)[0]);
    console.log('  Skipping POS tagger for now.');
  }
  
  console.log(`\nТайёр. Акнун \`node prisma/_tr-module-verify.mjs ${MODULE.order}\` -ро иҷро кунед.`);
