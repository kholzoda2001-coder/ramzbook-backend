...
async function main() {
  ...
  // 1. Delete existing Module 10 if it exists (due to partial failure)
  const existingMod10 = await prisma.module.findMany({
    where: { courseId: course.id, order: 10 }
  });

  for (const m of existingMod10) {
    console.log("Deleting duplicate/partial Module 10...");
