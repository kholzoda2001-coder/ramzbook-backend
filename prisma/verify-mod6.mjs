...
async function checkModule6() {
  const course = await prisma.course.findFirst({
    where: { targetLanguage: { code: 'en' }, nativeLanguage: { code: 'tg' }, level: 'A1' }
  });
  const m = await prisma.module.findFirst({ where: { order: 6, courseId: course.id }, include: { lessons: true } });
  if (!m) { console.log('Mod 6 not found'); return; }
...
checkModule6().catch(e=>console.error(e)).finally(()=>prisma.$disconnect());
