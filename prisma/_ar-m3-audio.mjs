// Аудио барои он чизе, ки дар Модули 3 НАВ ё ИВАЗ шуд.
//
//   1. Калимаҳои нави бе аудио (3 рақам: واحد، اثنان، ثلاثة).
//   2. 4 матн + сатри муколамае ки калимааш иваз шуд (عاماً → سنةً).
//
...
//
//   node prisma/_ar-m3-audio.mjs --dry
//   node prisma/_ar-m3-audio.mjs
...
const [mod] = await q(`SELECT id FROM "Module" WHERE "courseId"=$1 AND "order"=2`, [course.id]);
...
// Сатри муколамае ки калимааш иваз шуд (`عَاماً` → `سَنَةً`).
const dlgLines = await q(
  `SELECT id, text AS txt FROM "DialogueLine"
    WHERE "dialogueId" IN (SELECT "dialogueId" FROM "Lesson" WHERE "moduleId"=$1 AND "dialogueId" IS NOT NULL)
      AND text LIKE '%سَنَةً%'`, [mod.id]);

const items = [
  ...words.map((r) => ({ ...r, table: 'Word' })),
  ...passages.map((r) => ({ ...r, table: 'ComprehensionExercise' })),
  ...dlgLines.map((r) => ({ ...r, table: 'DialogueLine' })),
];
console.log(`калимаи бе аудио: ${words.length} · матн: ${passages.length} · сатри муколама: ${dlgLines.length}`);
...
if (!git(['status', '--porcelain']).trim()) console.log('файли нав нест');
  ...
  git(['-c', 'user.name=RAMZ Content', '-c', 'user.email=help@ramz.tj',
    'commit', '-m', 'Arabic A1 Module 3: audio for new number words, rewritten passages and dialogue']);
