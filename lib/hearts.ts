 ...
 *   • A1 M1 → 1 дақ, M2 → 10, M3 → 20, M4–M5 → 30, M6+ → 45
 *   • A2 → 5 соат, B1 → 12 соат, B2 → 18 соат, C1+ → 24 соат
 *
 * ЧАРО ЗИНАПОЯ, на як рақам. Дар A1 ду хел корбар ҳаст:
 *  • модули 1–3 — ҳанӯз қарор надодааст, ки бимонад. Дилаш тамом шавад ва
 *    як соат интизор шавад, аксаран умуман барнамегардад: мо ҳам ӯро, ҳам
 *    тамоми даромади ояндаашро гум мекунем. Ин ҷо фишор зиён меорад.
 *  • модули 4+ — аллакай вақт гузоштааст. Ин ҷо интизорӣ ӯро намеронад,
 *    балки ба тамошои реклама ё обуна мебарад — яъне даромад медиҳад.
 * Пеш 30/60 дақ аз модули 2 сар мешуд — маҳз дар ҷои нодуруст. 45 дақиқаи
 * модули 6+ қариб ҳамон фишори пештараро нигоҳ медорад, вале дар ҷое ки
 * корбар аллакай пайваста аст.
 *
 * Ин бо кӯтоҳ кардани дарсҳо (ҳадди 8 калима, 26 машқ ба ҷои 39–64) як
 * маҷмӯа аст: хатогиҳо дар як нишаст камтар шуданд, пас нигоҳ доштани
 * фишори баланд дар модулҳои баъдӣ бехатар аст.
 */
export function regenMinutesFor(level: string, moduleIndex: number): number {
  ...
  switch (lvl) {
    case 'a1':
      if (idx <= 0) return 1;    // M1 — амалан бемаҳдуд
      if (idx === 1) return 10;  // M2
      if (idx === 2) return 20;  // M3
      if (idx <= 4) return 30;   // M4–M5
      return 45;                 // M6+ — корбар пайваста, фишор баргашт
    case 'a2':
      return 300;
    case 'b1':
      return 720;
