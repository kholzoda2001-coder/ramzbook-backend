...
/**
 * Чанд саҳифаи аввали китоби ҚУЛФ ройгон кушода мешавад.
 *
 * ⚠️ Ин рақам бояд бо `kFreePreviewPages`-и барнома
 * (`frontend/lib/screens/library_reader_screen.dart`) ЯКХЕЛА бошад. Агар
 * онҳо аз ҳам фарқ кунанд, хонанда матни «боз N саҳифа мондааст»-и
 * нодурустро мебинад — тест дар ҳарду тараф инро қулф мекунад.
 */
export const FREE_PREVIEW_PAGES = 5;

/**
 * Оё ин воҳид пешнамоиши САҲИФАВӢ дошта метавонад?
 *
 * ── Чаро на ҳама чиз ───────────────────────────────────────────────────────
 * Дархости сареҳи соҳиби маҳсулот (2026-09-02): «барои аудио ва видео
 * китобҳо ё ҳар чизи дигар бояд комилан премиюм бошанд, ягон саҳифа ё ягон
 * минуташ ройгон набошад». Пас `template`, `audio` ва `video` истисно
 * НЕСТАНД.
 *
 * ⚠️ EPUB низ не. Он ҷо «саҳифа» сутуни экран аст ва рақами устувор надорад:
 * дар телефони калон 3, дар хурд 8. «5 саҳифаи аввал» маъно намедиҳад.
 * Пешнамоиш танҳо барои китобе, ки саҳифаҳояш дар админ навишта шудаанд.
 *
 * ⚠️ Ҳамин қоида дар барнома такрор мешавад (`open_library_item.dart` →
 * `canPreview`), чунки он ҷо қарор ПЕШ аз дархост гирифта мешавад. Ду нусха
 * бояд як хел бошанд.
 */
export function canPreviewPages(item: {
  type: string;
  mediaUrl?: string | null;
}): boolean {
  if (item.type !== 'book') return false;
  return !item.mediaUrl || item.mediaUrl.trim().length === 0;
}
