...
async function clean() {
  ...
  if (modules.length > 0) {
    ...
    await sql`DELETE FROM "GrammarExample" WHERE "lessonId" IN (SELECT id FROM "Lesson" WHERE "moduleId"=${modId})`;
    await sql`DELETE FROM "ComprehensionQuestion" WHERE "lessonId" IN (SELECT id FROM "Lesson" WHERE "moduleId"=${modId})`;
    await sql`DELETE FROM "DialogueLine" WHERE "lessonId" IN (SELECT id FROM "Lesson" WHERE "moduleId"=${modId})`;
